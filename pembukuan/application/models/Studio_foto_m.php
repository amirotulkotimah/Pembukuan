 ...
 * 
 */
class Studio_foto_m extends CI_Model
{
	
	public function getAll(){ 
		$this->db->select('*');
		$this->db->from('tb_studio_foto');
		$this->db->join('tb_kategori', 'tb_kategori.id_kategori = tb_studio_foto.kategori', 'LEFT');
	...
	public function update_data($id_studio,$data) {
	    $this->db->where('id_studio',$id_studio);
	    return $this->db->update('tb_studio_foto',$data);
  	}
    
    public function hapus_data($id_studio, $where, $table) {
	    $this->db->where('id_studio',$id_studio);
	    $this->db->where($where);
	...
	public function get_id($id_studio)
    {
    	$this->db->where('id_studio',$id_studio);
    	return $this->db->get('tb_studio_foto');
    }
	...
	public function hitung_debit(){
		...
		$this->db->where('MONTH(tanggal)', date('m'));
   		$query = $this->db->get('tb_studio_foto');
   		if($query->num_rows()>0){
    ...
    function saldo_awal(){
   	...
   	public function delete($id_studio){        
		$this->db->where_in('id_studio', $id_studio);  
		$this->db->delete('tb_studio_foto');  
	}

	function get_studio_list($limit, $start){
		$this->db->join('tb_kategori', 'tb_kategori.id_kategori = tb_studio_foto.kategori', 'LEFT');
		$this->db->order_by('tanggal','ASC');
		$this->db->where('MONTH(tanggal)', date('m'));
        $query = $this->db->get('tb_studio_foto', $limit, $start);
        return $query;
    }
